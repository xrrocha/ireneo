 ...
 */
import { isSerializedClassInstance } from "./instance-reconstructor.js";
// ============================================================================
// Type Guards
// ============================================================================
/**
...
const deserializeFunction = (obj) => {
    const fn = new Function(`return (${obj.sourceCode})`)();
    const wrappedFn = fn;
 ...
 */
const deserializeDate = (obj, reconstructValue, refs) => {
    ...
    const date = dateValue === null ? new Date('invalid') : new Date(dateValue);
    ...
    for (const [key, value] of Object.entries(obj)) {
        ...
        date[key] = reconstructValue(value, refs);
    }
    return date;
};
/**
 ...
 */
const isUnresolvedRefMarker = (value) => {
    return (typeof value === "object" && value !== null && "__unresolved_ref__" in value);
};
/**
 * Reconstruct a Map from its entries
 */
const deserializeMap = (obj, parsed) => {
    const map = new Map();
    for (const [key, value] of obj.entries) {
        // Recursively deserialize keys and values
        const deserializedKey = typeof key === 'object' && key !== null && key.__type__
            ? deserializers[key.__type__]?.(key, parsed) || key
            : key;
        const deserializedValue = typeof value === 'object' && value !== null && value.__type__
            ? deserializers[value.__type__]?.(value, parsed) || value
            : value;
 ...
 */
const deserializeSet = (obj, parsed) => {
    const set = new Set();
...
const deserializers = {
    ...
    date: (obj, parsed) => {
        ...
        const simpleReconstruct = (val, _refs) => {
            // Handle references
            if (val && typeof val === 'object' && '__type__' in val) {
                const handler = deserializers[val.__type__];
    ...
    ref: (obj) => ({ __unresolved_ref__: obj.path }),
 ...
 */
function deserializeTwoPass(parsed, resolveRef, classRegistry) {
    ...
    function traverse(obj, path = []) {
        ...
        if (obj.__type__ && deserializers[obj.__type__]) {
            ...
            if (!handler)
                return obj; // Should never happen, but satisfies TypeScript
        ...
        if (classRegistry && isSerializedClassInstance(obj)) {
            ...
            if (!Constructor) {
                throw new Error(`Cannot reconstruct instance of class "${className}" - class not registered. ` +
 ...
 * ```
 */
export function deserializeSnapshot(json, classRegistry) {
     ...
     */
    const resolveRef = (path, root) => {
        let target = root;
        for (const segment of path) {
            target = target[segment];
            if (target === undefined) {
                throw new Error(`Cannot resolve snapshot ref path: ${path.join(".")}`);
            }
        }
        return target;
    };
 ...
 * ```
 */
export function deserializeEventValue(value, memoryRoot, classRegistry) {
     ...
     */
    const resolveRef = (path, valueRoot) => {
        ...
        for (const segment of path) {
            ...
            if (next === undefined) {
                ...
                throw new Error(`Cannot resolve event value ref path: ${path.join(".")} ` +
 ...
 */
export const reconstructValue = (value, root, seen = new WeakMap(), classRegistry) => {
    ...
    if (hasTypeMarker(value)) {
        ...
        switch (typedValue.__type__) {
            case "function":
                return deserializeFunction(typedValue);
            case "date": {
                // Create wrapper that matches expected signature
                const refs = root;
                const reconstruct = (val, _refs) => reconstructValue(val, root, seen, classRegistry);
                return deserializeDate(typedValue, reconstruct, refs);
            }
            case "regexp":
                return deserializeRegExp(typedValue);
            case "bigint":
                return deserializeBigInt(typedValue);
            case "symbol":
                return deserializeSymbol(typedValue);
            case "ref": {
                const refValue = typedValue;
                let target = root;
                for (const segment of refValue.path) {
                    target = target[segment];
                    if (target === undefined) {
                        throw new Error(`Cannot resolve ref path: ${refValue.path.join(".")}`);
                    }
                }
                return target;
            }
            case "map": {
                const mapValue = typedValue;
                const map = new Map();
                // Add to seen before recursing (handles cycles)
                seen.set(value, map);
                for (const [k, v] of mapValue.entries) {
                    const key = reconstructValue(k, root, seen, classRegistry);
                    const val = reconstructValue(v, root, seen, classRegistry);
                    map.set(key, val);
                }
                return map;
            }
            case "set": {
                ...
                for (const v of setValue.values) {
                    set.add(reconstructValue(v, root, seen, classRegistry));
                }
                return set;
            }
            case "circular":
                ...
                throw new Error("Encountered explicit circular marker - this indicates a serialization issue");
        }
    }
    // Arrays
    if (Array.isArray(value)) {
        ...
        for (const v of value) {
            result.push(reconstructValue(v, root, seen, classRegistry));
        }
        return result;
    }
    ...
    for (const key in value) {
        if (Object.prototype.hasOwnProperty.call(value, key)) {
            result[key] = reconstructValue(value[key], root, seen, classRegistry);
        }
    }
    // Class instance reconstruction
