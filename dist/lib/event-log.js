 ...
 */
export const createInMemoryEventLog = () => {
    const events = [];
    return {
        append: async (event) => {
            events.push(event);
 ...
 */
export const createFileEventLog = async (filepath) => {
    ...
    try {
        ...
        throw new Error('FileEventLog requires Node.js fs and readline modules. ' +
 ...
 */
export const createIndexedDBEventLog = (dbName = 'ireneo', storeName = 'events') => {
    let db;
    let closed = false;
    ...
    const assertNotClosed = () => {
        if (closed) {
            throw new Error(`EventLog (${storeName}) has been closed and cannot be used. ` +
 ...
 * @returns Event log with append/getAll/clear methods
 */
export const createLocalStorageEventLog = (key = 'ireneo-events') => {
