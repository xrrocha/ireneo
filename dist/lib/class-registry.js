...
class ClassRegistryImpl {
    ...
    register(constructor, name) {
        ...
        if (!className) {
            throw new Error("Cannot register class without a name. " +
                "Provide an explicit name or use a named class.");
        }
        const existing = this.registry.get(className);
        if (existing && existing !== constructor) {
            throw new Error(`Class name "${className}" is already registered with a different constructor. ` +
 ...
 */
export function createClassRegistry(classes) {
