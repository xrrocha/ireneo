...
async function importLog() {
    ...
    fileInput.onchange = async () => {
        ...
        try {
            ...
            const shouldImport = confirm(`Import ${importedEvents.length} events from "${file.name}"?`);
            if (!shouldImport) {
                return; // User cancelled
            }
            // Second confirmation: Replace or merge?
            const shouldReplace = confirm(`How should we import?\n\n` +
            ...
            await render();
            showStatus(`Imported ${importedEvents.length} events (${shouldReplace ? 'replaced' : 'merged'})`, 'success');
        }
        catch (err) {
    ...
    try {
        ...
        list.innerHTML = recent.map(event => {
            ...
            if (event.type === 'SET' && 'value' in event) {
                valueStr = `<div class="event-value">value: ${formatValue(event.value)}</div>`;
            }
            else if (event.type === 'ARRAY_PUSH' && 'items' in event) {
                valueStr = `<div class="event-value">pushed: ${formatValue(event.items[0])}</div>`;
            }
            else if (event.type === 'ARRAY_SPLICE' && 'start' in event) {
                valueStr = `<div class="event-value">index: ${event.start}, deleted: ${event.deleteCount}</div>`;
            }
            return `
...
function setupEventListeners() {
    ...
    window.toggleTodoAsync = (id) => toggleTodo(id).catch(console.error);
    window.deleteTodoAsync = (id) => deleteTodo(id).catch(console.error);
}
// ===== Theme =====
