          ...
          return function(...args: any[]) {
            ...
            if (methodName === 'push' || methodName === 'unshift' ||  methodName === 'splice') {
              wrappedArgs = args.map((arg, idx) => {
                ...
                if (proxyCache.has(arg)) return proxyCache.get(arg);
                if (targetCache.has(arg)) return arg; // Already a transaction proxy
        ...
        if (methodsReturningValues.includes(methodName)) {
          return function(...args: any[]) {
            ...
            if (deltaManager.has(keyPathKey)) {
              ...
              // Wrap if object
              if (isObject(deltaValue)) {
                if (proxyCache.has(deltaValue)) return proxyCache.get(deltaValue);
                if (targetCache.has(deltaValue)) return deltaValue;
            ...
            if (result !== null && result !== undefined && isObject(result)) {
              ...
              if (targetCache.has(result)) return result;
        ...
        if (mutatingMethods.includes(methodName)) {
          return function(...args: any[]) {
            ...
            if (deltaManager.has(setPathKey)) {
              setCopy = deltaManager.get(setPathKey) as Set<any>;
            } else {
              // First mutation - make a copy
              setCopy = new Set(obj);
